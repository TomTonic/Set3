...
func TestSet3Rehash(t *testing.T) {
	...
	set := FromArray(data)
	initialExpectedGroups := int(groupCountFor(calcReqNrOfGroups(uint32(len(data) * 7 / 5)))) //nolint:gosec
	...
	rehashToCapacityExpected := int(groupCountFor(calcReqNrOfGroups(200)))
	...
	set.Rehash()
	rehashExpected := int(groupCountFor(calcReqNrOfGroups(set.Size())))
...
// --- Element types reachable only through the public API -------------------
