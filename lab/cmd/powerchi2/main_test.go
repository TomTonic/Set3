...
// that the empirical (simulated) power is close to the analytic power, i.e.
// that the closed-form power calculation this tool reports is not simply wrong.
//
// The two numbers never match exactly. mcPower draws from rtcompare.CPRNG,
// which is crypto/rand-backed and explicitly not reproducible, and the analytic
// value is an asymptotic noncentral chi-square approximation that runs about
// one percentage point above the simulation at this sample size. Measured over
// repeated runs the gap sits between 0.007 and 0.013, so the tolerance below is
// wide enough to never flake and still narrow enough to catch a broken
// formula, which would be off by far more.
func TestMonteCarloComparison(t *testing.T) {
	...
	// Covers the approximation gap plus Monte Carlo sampling noise; see above.
	if math.Abs(mc-analytic) > 0.025 {
