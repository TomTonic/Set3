...
func TestRandomOps(t *testing.T) {
	...
	for _, cfg := range randOpsConfig {
		for iterations := 0; iterations < 200; iterations++ {
			sets := make([]*set3.Set3[uint32], cfg.numSets)
...
func fillAllSets(numSets, setSize, setVar, mod int, sets []*set3.Set3[uint32]) {
	for i := 0; i < numSets; i++ {
		sets[i] = set3.Empty[uint32]()
...
func deleteSomeElementsAndAddSomeNewOnes(numSets, mod int, sets []*set3.Set3[uint32]) {
...
func deleteSomeSetsCompletely(numSets int, sets []*set3.Set3[uint32]) {
...
func refillAllSets(numSets, setSize, setVar, mod int, sets []*set3.Set3[uint32]) {
	for i := 0; i < numSets; i++ {
...
func supersetTests(numSets, mod int, superset *set3.Set3[uint32], sets []*set3.Set3[uint32], t *testing.T) {
	for i := 0; i < numSets; i++ {
		...
		newSet := set3.FromArray([]uint32{rand.Uint32() % uint32(mod), rand.Uint32() % uint32(mod), rand.Uint32() % uint32(mod),
			rand.Uint32() % uint32(mod), rand.Uint32() % uint32(mod), rand.Uint32() % uint32(mod), rand.Uint32() % uint32(mod)})
