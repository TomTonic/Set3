...
func TestSet3Fill(t *testing.T) {
	...
	for _, cfg := range config {
		...
		for i := 0; i < cfg.rounds; i++ {
			...
			sets := make([]*set3.Set3[uint64], cfg.itersPerRoundFill)
			var startMem, endMem runtime.MemStats
			runtime.GC()
			runtime.ReadMemStats(&startMem)
			startTime := time.Now().UnixNano()
			for j := 0; j < cfg.itersPerRoundFill; j++ {
				set := set3.EmptyWithCapacity[uint64](uint32(uint64(cfg.initSetSize))) //nolint:gosec
		...
		fmt.Printf("Set3;Fill={BenchMem{BenchTime{set3.EmptyWithCapacity[uint64](%d) + %d*Add(uint64)}}};%d;%.3f;%.3f;%.3f\n", cfg.initSetSize, cfg.finalSetSize, cfg.finalSetSize, 0.0, medTime, medMem)
	}
}
	...
	for _, cfg := range config {
		for seedUp := 0; seedUp < 10; seedUp++ {
			for round := 0; round < 10; round++ {
				...
				resultSet := set3.FromArray(sdd.setValues)
				// Force garbage collection
				runtime.GC()
				// Give the garbage collector some time to complete
...
func TestSet3Find(t *testing.T) {
	...
	for _, cfg := range config {
		...
		for i := 0; i < cfg.rounds; i++ {
			...
			runtime.GC()
			runtime.ReadMemStats(&startMem)
			currentSet := set3.FromArray(currentSdd.setValues)
			runtime.GC()
			runtime.ReadMemStats(&endMem)
		...
		fmt.Printf("Set3;Find={BenchMem{set3.FromArray([%d]uint64)} + BenchTime{Contains(uint64)}};%d;%.3f;%.3f;%.3f\n", cfg.finalSetSize, cfg.finalSetSize, hitRea, medTime, medMem)
	}
}
