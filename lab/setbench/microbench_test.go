...
func TestMemoryFootprintSet(t *testing.T) {
	...
	for n := 10; n <= 350_000; n += 20 {
		b1 := testing.Benchmark(func(b *testing.B) {
			...
			m := set3.EmptyWithCapacity[int](uint32(n))
			require.NotNil(b, m)
		})
...
// genStringData builds count random strings of the given size from a
// deterministic source, so a benchmark run is reproducible.
