...
func runSet3Workload[T comparable](keys, hits, misses []T) uint64 {
	s := set3.EmptyWithCapacity[T](uint32(len(keys))) //nolint:gosec
	for _, k := range keys {
		s.Add(k)
	}
...
func runSet3SteadyWorkload[T comparable](s *set3.Set3[T], keys, hits, misses []T) uint64 {
	var hitCount uint64
	for _, k := range hits {
		if s.Contains(k) {
			hitCount++
...
func runSetVsMapRtcompare[T comparable](
	...
	v1 := set3.EmptyWithCapacity[T](uint32(len(keys))) //nolint:gosec
	...
	case "steady", "steady-state", "steady-state-only":
		...
		steadySet := set3.EmptyWithCapacity[T](uint32(len(keys))) //nolint:gosec
		for _, k := range keys {
			steadySet.Add(k)
		}
...
// TestRtcompare_Set3_vs_NativeMap compares set3.Set3[T] against map[T]struct{}
