...
import (
	...
	"github.com/TomTonic/Set3/lab/hashbench"
...
var hashFunctionsForUint8 = hashFunctionsForType{
	typesDescr:      "i08",
	implementations: []hashing.HashFunction{hashing.SwirlByte, hashalt.XorShift64Star08, hashalt.HashI08SM, hashalt.HashI08WH, hashalt.HashI08MH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 32)
		go func() {
			for i := range 256 {
...
var hashFunctionsForUint16 = hashFunctionsForType{
	typesDescr:      "i16",
	implementations: []hashing.HashFunction{hashing.HashI16SM, hashalt.HashI16WH, hashalt.HashI16MH, hashalt.XorShift64Star16},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			n := limit
			x := hashbench.XorShift16Star{State: rtcompare.NewCPRNG(16).Uint16() | 1} // non-zero random seed
			if n == 0 || n >= 1<<16 {
				n = 1<<16 - 1
			}
			for range n {
...
var hashFunctionsForUint32 = hashFunctionsForType{
	typesDescr:      "i32",
	implementations: []hashing.HashFunction{hashalt.HashI32SM, hashalt.HashI32WH, hashing.HashI32WHdet, hashalt.HashI32MH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			n := limit
			x := hashbench.XorShift32Star{State: rtcompare.NewCPRNG(16).Uint32() | 1} // non-zero random seed
			if n == 0 || n >= 1<<32 {
				n = 1<<32 - 1
			}
			for range n {
...
var hashFunctionsForUint64 = hashFunctionsForType{
	typesDescr:      "i64",
	implementations: []hashing.HashFunction{hashalt.HashI64SM, hashalt.HashI64WH, hashing.HashI64WHdet, hashalt.HashI64MH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			n := limit
...
var hashFunctionsForFloat32 = hashFunctionsForType{
	typesDescr:      "f32",
	implementations: []hashing.HashFunction{hashing.HashF32SM, hashalt.HashF32MH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			n := limit
...
var hashFunctionsForFloat64 = hashFunctionsForType{
	typesDescr:      "f64",
	implementations: []hashing.HashFunction{hashalt.HashF64SM, hashalt.HashF64MH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			n := limit
...
var hashFunctionsForString = hashFunctionsForType{
	typesDescr:      "str",
	implementations: []hashing.HashFunction{hashing.HashString, hashalt.HashStringWH, hashalt.HashStringMH},
	samples: func(limit uint64) <-chan any {
		ch := make(chan any, 8192)
		go func() {
			defer close(ch)
			...
			rng16 := hashbench.XorShift16Star{State: cprng.Uint16() | 1} // non-zero random seed
			for range 1<<16 - 1 {
				if count >= limit {
					return
				}
			...
			count++
			rng24 := hashbench.XorShift24Star{State: cprng.Uint32() | 1} // non-zero random seed
			for range 1<<24 - 1 {
				if count >= limit {
					return
				}
			...
			count++
			rng32 := hashbench.XorShift32Star{State: cprng.Uint32() | 1} // non-zero random seed
	...
	for _, hft := range hashFunctionsForAllTypes {
		for _, hf := range hft.implementations {
			...
			t.Run(hft.typesDescr+"/"+shortname, func(t *testing.T) {
				for numGroups := range hashbench.FilteredNumbers(numGroupSizes) {
