...
// The mixer is then either hashing.Splitmix64(seed^widened) or hashalt.WH32DetExtMul(widened, seed).
...
func calcNextGroupCountLocal(currentGroupCount uint32) uint32 {
	...
	p := hashbench.NextPrime(uint64(n))
	return uint32(p)
}
...
func hashForCandidate32(c rankCandidate32, u uint32, seed uint64) uint64 {
	...
	if c.useWH32 {
		return hashalt.WH32DetExtMul(widened, seed)
	}
...
//	prior tests). Other constants are exploratory hashalt.
...
func buildGroupCountsSet3Rank() []uint32 {
	...
	usualStartNumber := hashbench.NextPrime(uint64(calcReqNrOfGroupsLocal(21)))
	current := uint32(usualStartNumber)
	for range groupCountRehashPrefix {
		counts = append(counts, current)
		current = uint32(hashbench.NextPrime(uint64(calcNextGroupCountLocal(current))))
	}

	// Add extra sizes from the filtered generator used in existing comparison tests.
	for v := range hashbench.FilteredNumbers(groupCountFilteredHint) {
		// Normalize to prime counts to stay aligned with Set3's real table sizing.
		counts = append(counts, uint32(hashbench.NextPrime(uint64(v))))
