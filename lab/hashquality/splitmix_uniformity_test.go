...
// Uniformity and head-to-head comparisons for splitmix64 against the WyHash
// variants. These four tests used to sit in this file commented out, which
// meant no compiler or linter ever looked at them; they are live again now
// that the set3lab build tag keeps them out of the everyday build.
package hashquality
...
func Test32BitHasherUniformDistribution_Lower7Bits(t *testing.T) {
	...
	for i := range N {
		v1 := hashalt.WH32(uint32(i), 0xC0FFEE1234567890)
		countsWh32[v1&mask]++
	...
	t.Logf("lower 7 bits uniformity test: samples=%s buckets=%s mean=%s", hashbench.Pow2String(uint64(N)), hashbench.Pow2String(buckets), hashbench.Pow2String(uint64(mean)))
...
func TestCompareWh64Splitmix64(t *testing.T) {
	...
	s2 := run("wh64      ", func(v, seed uint64) uint64 { return hashalt.WH64(v, seed) })
...
func TestCompareWh64Splitmix64_MultiBuckets(t *testing.T) {
	...
	bucketsCh := hashbench.FilteredNumbers(bucketCases)
	for b := range bucketsCh {
		...
		cvW, maxDevW, collW, avgHdW, durW := run(func(v, seed uint64) uint64 { return hashalt.WH64(v, seed) })
